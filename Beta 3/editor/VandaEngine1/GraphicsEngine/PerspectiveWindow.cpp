	...
	for ( int n=1; n<=argc; ++n )
	{
		...
		for( CUInt i = 0 ; i < g_engineStaticSounds.size(); i++ )
		{
			if( Cmp( g_engineStaticSounds[i]->GetName(),  lua_tostring(L, n) ) )
			{
				...
				sprintf(temp, "%s%s%s", "\nSound '", g_engineStaticSounds[i]->GetName(), ".ogg' will be played continuously.");
		...
		{
			CChar temp[MAX_NAME_SIZE];
			sprintf(temp, "%s%s%s", "\nCoudn't find Sound '", lua_tostring(L, n), ".ogg' to be played.");
	...
	for (int n = 1; n <= argc; ++n)
	{
		...
		for (CUInt i = 0; i < g_engineStaticSounds.size(); i++)
		{
			if (Cmp(g_engineStaticSounds[i]->GetName(), lua_tostring(L, n)))
			{
				...
				//g_soundSystem->PlayALSound( *(g_engineStaticSounds[i]->m_source) );
				CChar temp[MAX_NAME_SIZE];
				sprintf(temp, "%s%s%s", "\nSound '", g_engineStaticSounds[i]->GetName(), ".ogg' will be played once.");
				PrintInfo(temp, COLOR_YELLOW);
				foundTarget = CTrue;
				break;
			}
	...
	for ( int n=1; n<=argc; ++n )
	{
		...
		for( CUInt i = 0 ; i < g_engineStaticSounds.size(); i++ )
		{
			if( Cmp( g_engineStaticSounds[i]->GetName(),  lua_tostring(L, n) ) )
			{
				...
				CChar temp[MAX_NAME_SIZE];
				sprintf(temp, "%s%s%s", "\nSound '", g_engineStaticSounds[i]->GetName(), ".ogg' will be paused.");
				PrintInfo(temp, COLOR_YELLOW);
				foundTarget = CTrue;
				break;
			}
		...
		{
			CChar temp[MAX_NAME_SIZE];
			sprintf(temp, "%s%s%s", "\nCoudn't find Sound '", lua_tostring(L, n), ".ogg' to be paused.");
			PrintInfo(temp, COLOR_RED);
		}
	...
	for ( int n=1; n<=argc; ++n )
	{
		...
		for( CUInt i = 0 ; i < g_engineStaticSounds.size(); i++ )
		{
			if( Cmp( g_engineStaticSounds[i]->GetName(),  lua_tostring(L, n) ) )
			{
				...
				CChar temp[MAX_NAME_SIZE];
				sprintf(temp, "%s%s%s", "\nSound '", g_engineStaticSounds[i]->GetName(), ".ogg' will be stopped.");
				PrintInfo(temp, COLOR_YELLOW);
				foundTarget = CTrue;
				break;
			}
		...
		{
			CChar temp[MAX_NAME_SIZE];
			sprintf(temp, "%s%s%s", "\nCoudn't find Sound '", lua_tostring(L, n), ".ogg' to be stopped.");
			PrintInfo(temp, COLOR_RED);
		}
...
CBool CMultipleWindows::InitAll()
{
	...
	if( m_loadData ) 
	{
		...
		if (!m_nx->initNx(5.0, 5.0, 5.0, g_physXProperties.m_fGravityY, g_physXProperties.m_fCapsuleRadius, g_physXProperties.m_fCapsuleHeight, g_physXProperties.m_fCharacterSlopeLimit, g_physXProperties.m_fCharacterSkinWidth, g_physXProperties.m_fCharacterStepOffset))
		{
...
CVoid CMultipleWindows::DrawPerspective()
{
	...
	if (!g_useOldRenderingStyle && g_shadowProperties.m_enable && g_render.UsingShadowShader() && g_render.m_useDynamicShadowMap && g_options.m_enableShader)
	{
		...
		if( !instanceCamera )
		{
			...
			cam_dir[1] = g_camera->m_perspectiveCharacterPos.y + (g_physXProperties.m_fCapsuleHeight / 2.f ) + g_camera->m_perspectiveCameraTilt - cam_pos[1];
...
CVoid CMultipleWindows::RenderQueries(CBool init)
{
	if (m_bQuery || init)
	{
		...
		if (m_calculateDistance || init)
		{
			...
			for (CUInt i = 0; i < g_instancePrefab.size(); i++)
			{
				if (g_currentCameraType == eCAMERA_DEFAULT_PHYSX && g_instancePrefab[i]->GetIsTrigger())
					continue;

				if (g_currentCameraType == eCAMERA_DEFAULT_PHYSX && Cmp(g_instancePrefab[i]->GetPrefab()->GetPackageName(), "Vanda_Basics") && Cmp(g_instancePrefab[i]->GetPrefab()->GetPrefabName(), "Character"))
					continue;
